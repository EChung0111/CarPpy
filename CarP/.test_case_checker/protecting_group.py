...
class ConformerTest:
    ...
    @staticmethod
    def coord_center_change(center_array, coords):

        new_coords = []
        for coord, cent in zip(coords, center_array):
            new_coord = coord - cent
            new_coords.append(new_coord)

        return new_coords

    @staticmethod
    def coord_uncenter_change(center_array, coords):
        new_coords = []
        for coord, cent in zip(coords, center_array):
            new_coord = coord + cent
            new_coords.append(new_coord)
    ...
    @staticmethod
    def set_dihedral(atom1, atom2, atom3, atom4, xyz_array, angle, conn_mat):
        ...
        for node in nodes:
            if atom4 == node:
                carried_atoms.append(node)
            elif node not in bad_nodes and pred[node] not in bad_nodes:
                carried_atoms.append(node)
            else:
                bad_nodes.append(node)

        new_coords = []
        for node in carried_atoms:

            node_index = list(xyz_array[:, 0]).index(node)

            node_coords = []
            for c_index in range(1, 4):
                node_coords.append(float(xyz_array[node_index, c_index]))

            node_cent = ConformerTest.coord_center_change(atom2_coords, node_coords)
            rot_node = np.dot(rotation_matrix, node_cent)
            new_node_coords = ConformerTest.coord_uncenter_change(atom2_coords, rot_node)
            new_node_coords.insert(0, node)
            new_node_coords = np.array(new_node_coords, dtype=str)
            new_coords.append(new_node_coords)
    ...
    def get_pg_dihedral(pg_list,xyz_array):
        dihedral_list = []
        for pg in pg_list:
            atoms = [atom for atom in pg.values()]
            dihedral = ConformerTest.dihedral_angle(atoms[0], atoms[1], atoms[2], atoms[3], xyz_array)
            dihedral_list.append(dihedral)

        return dihedral_list

    def set_pg_dihedral(pg_list,xyz_array, angle_list, conn_mat):

        coords_list = []
        for pg,angle in zip(pg_list,angle_list):
            atoms = [atom for atom in pg.values()]
            coords = ConformerTest.set_dihedral(atoms[0], atoms[1], atoms[2], atoms[3], xyz_array, angle, conn_mat)
...
if __name__ == "__main__":
    ...
    for file in ConformerTest.get_files(working_dir):
        ...
        if 'geometry.xyz' in file:
            ...
            dihedral =  ConformerTest.get_pg_dihedral(pg_list,xyz_array)
            print("PG DIhedral: ", dihedral)

            coords = ConformerTest.set_pg_dihedral(pg_list,xyz_array,[60], conn_mat)

            for pg in coords:
                for row in pg:
                    atom = row[0]
                    atom_index = list(xyz_array[:, 0]).index(atom)
            ...
            with open('new_geom.xyz','w') as file:
                ...
                for row in xyz_array:
                    ...
                    file.write(f"{atom}       {row[1]}       {row[2]}       {row[3]}\n")
